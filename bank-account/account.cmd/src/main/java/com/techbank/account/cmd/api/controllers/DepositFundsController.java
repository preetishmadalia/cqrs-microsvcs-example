...
public class DepositFundsController {
    ...
    public ResponseEntity<BaseResponse> depositFunds(
        ...
        try {
            ...
            return new ResponseEntity<>(new BaseResponse("Amount deposit completed successfully!"), HttpStatus.OK);
        } catch (IllegalStateException ise) {
            log.warn(MessageFormat.format("Cannot complete funds deposit at this time : {0}", ise.getMessage()));
            return new ResponseEntity<>(new BaseResponse(MessageFormat.format("Cannot complete funds deposit at this time : {0}", ise.getMessage())), HttpStatus.BAD_REQUEST);
        } catch (Exception ex) {
            ex.printStackTrace();
